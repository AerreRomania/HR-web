// RaportSuplimentareDepartament.js
function Lista()  
{
 
        $get('up').style.display = '';
        WbmOlimpias.RaportSuplimentareDepartamentWS.RaportSuplimentareDepartamentLista($get("ddlFiltruAn").value, $get("ddlFiltruLuna").value, $get("tbFiltruDepartament").value, $get("ddlFiltruEchipaLinie").value, $get("ddlFiltruMansione").value, SucceededCallback, FailedCallback);
 
 
}

//function ListaChanged() {
//    $get('up').style.display = '';
//    WbmOlimpias.RaportSuplimentareDepartamentWS.RaportSuplimentareDepartamentLista($get("ddlFiltruAn").value, $get("ddlFiltruLuna").value, $get("tbFiltruDepartament").value, $get("ddlFiltruEchipaLinie").value, $get("ddlFiltruMansione").value, SucceededCallback, FailedCallback);
...
function bExcel_Click() {
    $get('up').style.display = '';
    WbmOlimpias.RaportSuplimentareDepartamentWS.RaportSuplimentareDepartamentExcel($get("ddlFiltruAn").value,$get("ddlFiltruLuna").value, $get("tbFiltruDepartament").value, SucceededCallback, FailedCallback);
}


function printExcel() {

    myTable = document.getElementById('tSuplimentareDepartamentOre');
...
function printData() {
    ...
    $('#tSuplimentareDepartamentOre').clone().appendTo("#imaginary-friend");
...
function printPDF() {
    ...
    html2canvas($("#tSuplimentareDepartamentOre")[0], {
    ...
    setTimeout(function () {
        pdf.save('Report Straordinario Departament.pdf');
    }, 0);
}
...
function SucceededCallback(result, userContext, methodName) {
    ...
    switch (methodName) {
        case ("RaportSuplimentareDepartamentLista"):
            {
                if (result.Eroare !== '') {
                    window.alert(result.Eroare);
                    return;
                }
                AfisareSuplimentareDepartament("tSuplimentareDepartamentOre", result.TabelaSuplimentareDepartamentOre.sort(function (a, b) {
        ...
        case ("RaportSuplimentareDepartamentExcel"):
            {
                if (result.Eroare !== '') {
                    window.alert(result.Eroare);
                    return;
...
function AfisareSuplimentareDepartament(NumeTabela, Tabela, NumarZile) {
    ...
    for (var item in Tabela) {
        ...
        for (var secondItem in Tabela[item].ValoriZilnice) {
            ...
            if (Tabela[item].ValoriZilnice[secondItem] === "")
                Tabela[item].ValoriZilnice[secondItem] = '&nbsp;';
             
            newcell.innerHTML = Tabela[item].ValoriZilnice[secondItem];
            newcell.title = Tabela[item].ValoriZilnice[secondItem];
        }
        ...
        //newcell.title = Tabela[item].Total;
        i++;
    }
    ...
    //PEKI CODE
    var table = $('#tSuplimentareDepartamentOre');
    ...
    var wizard = document.getElementById("tSuplimentareDepartamentOre");
    ...
    $('#tSuplimentareDepartamentOre tr:nth-last-child(2)').remove();
    $('#tSuplimentareDepartamentOre tr:nth-last-child(2)').remove();


    if ($('#ddlFiltruEchipaLinie').val() !== 'ALL') {
 
        var zadnjired = $('#tSuplimentareDepartamentOre tr').length - 5;
        $('#tSuplimentareDepartamentOre tr').eq(2).insertBefore($('#' + zadnjired + ''));
    }  
