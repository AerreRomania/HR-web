...
function Lista(Find)  
{
    ...
    var oFiltruNiveleStudii = new WbmOlimpias.FiltruNiveleStudiiObiect();
    oFiltruNiveleStudii.Find = Find;
    oFiltruNiveleStudii.FiltruNivelStudiu = $get("tbFiltruNivelStudiu").value;
    WbmOlimpias.NiveleStudiiWS.NiveleStudiiLista(oFiltruNiveleStudii, $get("tbPaginaCurenta").value, SucceededCallback, FailedCallback);
}
function Proprietati()
{
    ...
    {
        $get('up').style.display = '';
        WbmOlimpias.NiveleStudiiWS.NivelStudiuProprietati(Id, SucceededCallback, FailedCallback);
    }
}
function Adaugare()
{
    ...
    oNivelStudiu.Id = '';
    oNivelStudiu.NivelStudiu = $get("tbNivelStudiu").value;
    WbmOlimpias.NiveleStudiiWS.NivelStudiuAdaugare(oNivelStudiu, SucceededCallback, FailedCallback);
}
function Modificare()
{
    $get('up').style.display = '';
    var oNivelStudiu = new WbmOlimpias.NivelStudiuObiect();
    oNivelStudiu.Id = $get("hfId").value;
    oNivelStudiu.NivelStudiu = $get("tbNivelStudiu").value;
    WbmOlimpias.NiveleStudiiWS.NivelStudiuModificare(oNivelStudiu, SucceededCallback, FailedCallback);
}
function Stergere()
{
    ...
    {
        $get('up').style.display = '';
        WbmOlimpias.NiveleStudiiWS.NivelStudiuStergere($get("hfId").value, SucceededCallback, FailedCallback);
    }
}
function SucceededCallback(result, userContext, methodName) {
    $get('up').style.display = 'none';
    switch (methodName) {
        case ("NiveleStudiiLista"):
            {
                var mytable = $get("tNiveleStudii");
                ...
                for (var item in result.Tabela) {
                    ...
                    newcell.id = 'NivelStudiu_' + i.toString();
                    newcell.className = 'neselectat';
                    newcell.innerHTML = '&nbsp;' + result.Tabela[item].NivelStudiu;
                    newcell.title = result.Tabela[item].NivelStudiu;
        ...
        case ("NivelStudiuProprietati"):
            {
                $get("tbNivelStudiu").value = result.NivelStudiu;
                $get("tbFiltruNivelStudiu").focus();
                break;
            }
        case ("NivelStudiuAdaugare"):
            {
                if (result.Eroare == '')
                    Lista(result.Id);
                else
...
function RowSelect()
{
    $get("NivelStudiu_" + $get("hfRowIndex").value).className = 'selectat';
}
function RowUnselect()
{
    var obiect = $get("NivelStudiu_" + $get("hfRowIndex").value);
    if(obiect!=null)
...
function SetariValori() {
    if (window.dialogArguments && dialogArguments.location) {
        var Id = $get('hfId').value;
        var NivelStudiu = $get('tbNivelStudiu').value;
        var Argumente = new Array(Id, NivelStudiu);
...
function onKeyDown (eventElement) 
{
    ...
    switch (eventElement.keyCode) 
    {
        case 13:    // enter
            ...
            if(eventElement.target.id=='tbFiltruNivelStudiu')
                Lista('');
