// TipuriPostDeLucru.js
function Lista(Find)  
{
    $get('up').style.display = '';
    var oFiltruTipuriPostDeLucru = new WbmOlimpias.FiltruTipuriPostDeLucruObiect();
    oFiltruTipuriPostDeLucru.Find = Find;
    oFiltruTipuriPostDeLucru.FiltruTipPostDeLucru = $get("tbFiltruTipPostDeLucru").value;
    WbmOlimpias.TipuriPostDeLucruWS.TipuriPostDeLucruLista(oFiltruTipuriPostDeLucru, $get("tbPaginaCurenta").value, SucceededCallback, FailedCallback);
}
function Proprietati()
{
    ...
    {
        $get('up').style.display = '';
        WbmOlimpias.TipuriPostDeLucruWS.TipPostDeLucruProprietati(Id, SucceededCallback, FailedCallback);
    }
}
function Adaugare()
{
    ...
    oTipPostDeLucru.Id = '';
    oTipPostDeLucru.TipPostDeLucru = $get("tbTipPostDeLucru").value;
    WbmOlimpias.TipuriPostDeLucruWS.TipPostDeLucruAdaugare(oTipPostDeLucru, SucceededCallback, FailedCallback);
}
function Modificare()
{
    $get('up').style.display = '';
    var oTipPostDeLucru = new WbmOlimpias.TipPostDeLucruObiect();
    oTipPostDeLucru.Id = $get("hfId").value;
    oTipPostDeLucru.TipPostDeLucru = $get("tbTipPostDeLucru").value;
    WbmOlimpias.TipuriPostDeLucruWS.TipPostDeLucruModificare(oTipPostDeLucru, SucceededCallback, FailedCallback);
}
function Stergere()
{
    ...
    {
        $get('up').style.display = '';
        WbmOlimpias.TipuriPostDeLucruWS.TipPostDeLucruStergere($get("hfId").value, SucceededCallback, FailedCallback);
    }
}
function SucceededCallback(result, userContext, methodName) {
    $get('up').style.display = 'none';
    switch (methodName) {
        case ("TipuriPostDeLucruLista"):
            {
                var mytable = $get("tTipuriPostDeLucru");
        ...
        case ("TipPostDeLucruProprietati"):
            {
                $get("tbTipPostDeLucru").value = result.TipPostDeLucru;
                $get("tbFiltruTipPostDeLucru").focus();
                break;
            }
        case ("TipPostDeLucruAdaugare"):
            {
                if (result.Eroare == '')
                    Lista(result.Id);
                else
...
function RowSelect()
{
    $get("TipPostDeLucru_" + $get("hfRowIndex").value).className = 'selectat';
}
function RowUnselect()
{
    var obiect = $get("TipPostDeLucru_" + $get("hfRowIndex").value);
    if(obiect!=null)
...
function SetariValori() {
    if (window.dialogArguments && dialogArguments.location) {
        ...
        var Argumente = new Array(Id, TipPostDeLucru);
...
function onKeyDown (eventElement) 
{
    ...
    switch (eventElement.keyCode) 
    {
        case 13:    // enter
            ...
            if(eventElement.target.id=='tbFiltruTipPostDeLucru')
                Lista('');
