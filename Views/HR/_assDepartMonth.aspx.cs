...
public partial class Views_HR_assDepartMonth : System.Web.UI.Page
{
    ...
    private void LoadData()
    {
        ...
        DataTable _lastestDataInDB = helper.PkSelect("SELECT MONTH(Prezente.Data), YEAR(Prezente.Data) FROM Prezente WHERE Prezente.Id = (SELECT MAX(Prezente.Id) FROM prezente); ", "WbmOlimpiasConnectionString");
        ...
        {
             _data = helper.PkSelect("SELECT DISTINCT [R-Code], [Dipendente], [Mansione], [Linea]  FROM [_assDeptMonth]" +
        ...
        DataTable _ore = helper.PkSelect("SELECT [R-Code], [Day], [Ore]  FROM [_assDeptMonth] WHERE Departament='" + request + "' and Month=" + ddp_month.SelectedValue + " and Year=" + ddp_year.SelectedValue + "", "WbmOlimpiasConnectionString");
        if (!Page.IsPostBack)
        {
            _linie = _data.Copy();
