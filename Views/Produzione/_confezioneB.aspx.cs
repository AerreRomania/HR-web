...
public partial class Views_Produzione_confezioneB : System.Web.UI.Page
{
    ...
    {
        lbl_time.Text = "CONFEZIONE B LAYOUT - Loaded: " + DateTime.Now.ToLongTimeString() + " - ";
        ...
        foreach (var item in LineEffList)
        {
            ...
            if (line == "LINEA1")
            {
                LINEA1.InnerHtml = value;
            } else if (line == "LINEA2")
            {
                LINEA2.InnerHtml = value;
            }
            else if (line == "LINEA3")
            {
                LINEA3.InnerHtml = value;
            }
            else if (line == "LINEA4")
            {
                LINEA4.InnerHtml = value;
            }
            else if (line == "LINEA5")
            {
                LINEA5.InnerHtml = value;
            }
            else if (line == "LINEA6")
            {
                LINEA6.InnerHtml = value;
            }
            else if (line == "LINEA7")
            {
                LINEA7.InnerHtml = value;
            }
            else if (line == "LINEA8")
            {
                LINEA8.InnerHtml = value;
            }
            else if (line == "LINEA9")
            {
                LINEA9.InnerHtml = value;
            }
            else if (line == "LINEA10")
            {
                LINEA10.InnerHtml = value;
            }
            else if (line == "LINEA11")
            {
                LINEA11.InnerHtml = value;
            }
            else if (line == "LINEA12")
            {
                LINEA12.InnerHtml = value;
            }
        }
    ...
    public List<EmployeeEfficiency> GetEmployeeEfficiency(int sectorId)
    {
        ...
        using (var con = new SqlConnection(connectionString))
        {
            ...
            if (dr.HasRows)
            {
                var pauses = new CurrentJobPauseC[2];
                if (sectorId == 1)
                {
                    pauses[0] = new CurrentJobPauseC()
                    {
                        Start = new TimeSpan(10, 40, 0),
                        End = new TimeSpan(11, 0, 0)
                    };
                    pauses[1] = new CurrentJobPauseC()
                    {
